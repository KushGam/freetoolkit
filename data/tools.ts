...
export type Tool = {
  ...
  /** Long-form H2/H3 editorial sections (1200–2500 word target when combined). */
  sections?: ToolContentSection[];
  /** SEO strategy metadata from content blueprints. */
  seoBlueprint?: {
...
const privacyNote = "Your files are processed in your browser where possible. freetoolkitapp does not require sign-up, payment, or a server upload workflow for these tools.";
...
const coreTools: Tool[] = [
  {
    ...
    metaTitle: "Image Compressor Online Free",
  ...
  },
  {
    ...
    seo: [
      ...
      "freetoolkitapp keeps the process simple and accessible. You upload a JPG, the browser draws the image, and a PNG file is generated for download. There is no signup, no paid API, and no complicated settings. The conversion happens in your browser where supported, which makes it fast and secure for everyday use. This is helpful for students, freelancers, office workers, website owners, and anyone who needs a quick file-format change.",
      "For best results, use this converter when the requested output is specifically PNG or when your next tool works better with PNG files. If your goal is to make a photo smaller, an image compressor or WebP converter may be a better choice because PNG files can be larger. If your goal is transparency, remember that a JPG does not contain transparent areas. freetoolkitapp gives you the converted file quickly while keeping the decision about the best final format in your hands.",
  ...
  },
  {
    ...
    seo: [
      ...
      "This converter fits naturally with other freetoolkitapp image tools. Compress an image first if you need extra size reduction, resize it if the dimensions are too large, or convert from PNG to JPG if a platform does not accept WebP. For most modern web publishing workflows, WebP is a smart final format. freetoolkitapp keeps the process beginner-friendly while still giving professionals the control they need for repeat optimization work.",
  ...
  },
  {
    ...
    seo: [
      "The Split PDF tool helps you create a smaller PDF by selecting only the pages you need. It is useful when a long document contains one form, one chapter, one signed page, or one section that you want to share separately. Instead of sending the entire file, you can enter page ranges and download a new PDF. freetoolkitapp also shows the number of pages in the uploaded PDF, which makes the process easier and reduces mistakes.",
  ...
  },
  {
    ...
    metaTitle: "Reduce PDF File Size Online Free",
    ...
    seo: [
      ...
      "PDF compression depends on what is inside the file. A text-based PDF may shrink when unnecessary structure or metadata is removed. A scanned PDF, however, is often large because each page is an image. Rebuilding that kind of file may not reduce much unless the images themselves are downsampled or recompressed. freetoolkitapp avoids fake promises and explains this limitation clearly, so you know what to expect before relying on the output.",
      ...
      "For best results, start with PDFs that are not password protected. After downloading, open the optimized copy and confirm that pages display correctly. If you need to remove pages instead of compressing the whole file, use Split PDF or Extract Pages from PDF. If your document is made from many separate PDFs, Merge PDF can organize them first. freetoolkitapp gives you a practical set of browser PDF tools for everyday document handling.",
  ...
  },
  {
    ...
    seo: [
      ...
      "Before sharing the rotated PDF, open the downloaded file and check that the pages face the right way. If your document also includes extra pages, use Split PDF or Extract Pages from PDF after rotating. If you need to combine corrected pages with other documents, use Merge PDF. freetoolkitapp gives you practical PDF cleanup tools that work well for everyday document preparation.",
  ...
  },
  {
    ...
    metaTitle: "Extract Pages from PDF Online Free",
    ...
    seo: [
      "The Extract Pages from PDF tool helps you pull specific pages from a larger document and save them as a new PDF. This is useful when you need one receipt from a long statement, one signed page from a contract, a few pages from a handbook, or selected pages from a class packet. freetoolkitapp shows the page count after upload, so you can choose valid page numbers with confidence.",
...
export type TopLevelCategory = "AI Tools" | "PDF & Image" | "SEO Tools" | "Developer" | "Calculators";

export const topLevelCategories: TopLevelCategory[] = ["AI Tools", "PDF & Image", "SEO Tools", "Developer", "Calculators"];
...
export const topLevelCategoryIntros: Record<TopLevelCategory, string> = {
  "AI Tools":
    "AI productivity tools for resumes, emails, grammar, study notes, summarizing, paraphrasing, and interview prep—always review generated drafts before you submit them.",
  "PDF & Image":
    "High-traffic PDF and image utilities: merge, split, compress, convert, resize, crop, and watermark—built for browser-based document and media workflows.",
  "SEO Tools":
    "SEO utilities for meta tags, Open Graph, robots.txt, sitemaps, SERP previews, slugs, schema markup, and keyword density checks.",
  Developer:
...
export function getTopLevelCategory(tool: Tool): TopLevelCategory {
  if (tool.category === "AI Tools" || tool.slug === "ai-resume-cover-letter" || tool.slug === "resume-ats-checker") {
    return "AI Tools";
  }
...
export function getRelatedTools(tool: Tool) {
  ...
  if (tool.category === "AI Tools" || tool.slug === "resume-ats-checker") {
    const preferred = [
