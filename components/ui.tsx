...
export function Button({ className, ...props }: ButtonHTMLAttributes<HTMLButtonElement>) {
  return (
    <button
      className={cn(
        "inline-flex min-h-12 w-full items-center justify-center rounded-2xl bg-gradient-to-b from-brand-500 via-brand-600 to-brand-700 px-5 py-3 text-sm font-semibold tracking-tight text-white shadow-[0_16px_34px_rgba(127,29,29,0.24)] transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[0_22px_42px_rgba(127,29,29,0.3)] focus:outline-none focus:ring-4 focus:ring-brand-100 disabled:cursor-not-allowed disabled:translate-y-0 disabled:opacity-50 sm:w-auto",
        className
      )}
      {...props}
    />
...
export function Select({ className, ...props }: SelectHTMLAttributes<HTMLSelectElement>) {
  return (
    <select
      className={cn(
        "min-h-12 w-full rounded-xl border border-slate-200 bg-gradient-to-b from-white to-slate-50 px-4 py-3 text-sm font-normal text-slate-900 shadow-sm outline-none transition-all duration-200 focus:border-brand-500 focus:ring-4 focus:ring-brand-100",
        className
      )}
      {...props}
    />
...
export function PageHeader({
  ...
  return (
    <section className={cn("relative overflow-hidden rounded-[2rem] border border-slate-200 bg-[radial-gradient(circle_at_top_left,#fef2f2,transparent_40%),radial-gradient(circle_at_top_right,#ffffff,transparent_34%),linear-gradient(180deg,#ffffff,#fafafa)] p-6 text-center shadow-[0_28px_75px_rgba(15,23,42,0.08)] sm:p-9 lg:p-12", className)}>
      ...
      {description ? <div className="mx-auto mt-4 max-w-3xl text-base font-normal leading-8 text-slate-600 sm:text-lg">{description}</div> : null}
      {badges?.length ? (
...
export function EmptyState({ title, description }: { title: string; description?: string }) {
  return (
    <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50/70 p-6 text-center">
      <p className="text-base font-display font-bold tracking-tight text-slate-950">{title}</p>
...
export function ResultCard({ title, children, className }: { title?: string; children: React.ReactNode; className?: string }) {
  return (
    <Card className={cn("bg-gradient-to-b from-white to-slate-50/80", className)}>
      {title ? <h3 className="text-sm font-semibold uppercase tracking-wide text-brand-600">{title}</h3> : null}
...
export function CategoryCard({
  ...
  return (
    <Link href={href} prefetch={false} className="group rounded-2xl border border-slate-200 bg-gradient-to-b from-white to-slate-50/95 p-6 shadow-[0_12px_30px_rgba(15,23,42,0.08)] backdrop-blur transition-all duration-200 hover:-translate-y-1 hover:border-brand-200 hover:shadow-[0_22px_45px_rgba(15,23,42,0.14)] focus:outline-none focus:ring-4 focus:ring-brand-100">
      <div className="flex items-start justify-between gap-4">
